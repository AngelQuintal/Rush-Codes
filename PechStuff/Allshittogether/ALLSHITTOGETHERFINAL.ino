...
void IRAM_ATTR detectsMovement() {
  ...
  digitalWrite(buzzer, HIGH);//WE NEED TO USE A VARIBABLE FOR SOME REASON THE LED CRASHES
  startTimer = true;
  lastTrigger = millis();
}
...
void compare_user_stored(byte rfid_read[4]){//PUT THE ID HERE
      ...
      admin[2] == rfid_read[2] && admin[3] == rfid_read[3] )
  {
       ...
       digitalWrite(buzzer, HIGH);//OPENS DOOR
       //Sends a 1 to server
  ...
  else
  {
    ...
    digitalWrite(buzzer, LOW);//OPENS DOOR
    //Sends a 0 to server
